...
module.exports = {
  ...
  .addUserOption((option) => option.setName("member").setDescription("The member to change the nickname of").setRequired(true))
  ...
  async execute(interaction) {
    ...
    if(!interaction.member.permissions.has("MANAGE_NICKNAMES")) return interaction.reply({ content: "You do not have permission to use this command.", ephemeral: true })
    ...
    if (member.id === interaction.member.id) {
      ...
      .setFooter({ text: "https://mee8.ml" })
      .setColor(client.color.red)
      return interaction.reply({ embeds: [setnickerror] })
      .then(async () => {
        if (logging && logging.moderation.delete_reply === "true") {
          setTimeout(() => {
            interaction.deleteReply().catch(() => {});
    ...
    if (nickname) {
      try {
        ...
        const embed = new MessageEmbed()
        .setDescription(`${success} | **${oldNickname}**'s nickname was set to **${nick}**`)
        .setColor(client.color.green);
        interaction.reply({ embeds: [embed] })
        .then(async () => {
          if (logging && logging.moderation.delete_reply === "true") {
