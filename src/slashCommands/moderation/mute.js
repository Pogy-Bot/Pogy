...
module.exports = {
  ...
  .setDescription("Mute a person in the server!")
    ...
    .addStringOption((option) => option.setName("time").setDescription("For how much time you want to timeout for").setRequired(true))
    .addStringOption((option) => option.setName("reason").setDescription("The reason of the timeout")),
  async execute(interaction) {
    ...
    if(!time) {
      ...
      .setColor("RED")
      .setDescription(`${client.emoji.fail} | The time specified is not valid. It is necessary that you provide valid time.`)
    
    return interaction.reply({ embeds: [timevalid] })
      .then(async () => {
        if (logging && logging.moderation.delete_reply === "true") {
          setTimeout(() => {
            interaction.deleteReply().catch(() => {});
    ...
    const response = await member.timeout(time, reason)
      ...
      .setColor("GREEN")
      .setDescription(`${client.emoji.success} | ${member} has been timed out for ${ms(time, { long: true })}`)
      return interaction.reply({ embeds: [timeoutsuccess] })
      .then(async () => {
        if (logging && logging.moderation.delete_reply === "true") {
          setTimeout(() => {
            interaction.deleteReply().catch(() => {});
    ...
    if(member) {
      ...
      .setDescription(`${client.emoji.fail} | I cannot time out that member. Make sure that my role is above their role or that I have sufficient perms to execute the command.`)
      .setTimestamp()
