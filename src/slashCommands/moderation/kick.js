...
module.exports = {
    ...
    .addUserOption((option) => option.setName("member").setDescription("Person who you want to kick.").setRequired(true))
    .addStringOption((option) => option.setName("reason").setDescription("The reason of the kick")),
    async execute(interaction) {
        ...
        if(member === interaction.author) {
            ...
            return interaction.reply({ embeds: [kickerror] })
            .then(async () => {
                if (logging && logging.moderation.delete_reply === "true") {
                    setTimeout(() => {
                        interaction.deleteReply().catch(() => {})
        ...
        if(member) {
            ...
            .setColor("GREEN")
            .setDescription(`${client.emoji.success} | ${member} has been kicked. __**Reason:**__ ${reason || "No reason Provided"}`)
        ...
        if(member) {
            ...
            .setDescription(`${client.emoji.fail} | I cannot kick that member. Make sure my role is above their role or that I have sufficient perms to execute the command.`)
            .setTimestamp()
