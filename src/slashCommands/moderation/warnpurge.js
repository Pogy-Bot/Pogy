...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (
      logging &&
      logging.moderation.warn_action && logging.moderation.warn_action !== "1"
    ) {
    ...
    if (messages.size > 0) await interaction.channel.bulkDelete(messages, true);

    if (mentionedMember) {
      const embed = new MessageEmbed()
      .setDescription(`${success} | **${mentionedMember.user.tag}** has been warned, with **${
        messages.size
      }** messages purged.\n\n__**Reason:**__ ${reason}`)
